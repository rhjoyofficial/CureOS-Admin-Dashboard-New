...
@section('content')
    <div class="max-w-6xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Update invoice details</p>
                    </div>
            ...
            <!-- Form -->
            <form action="{{ route('admin.invoices.update', $invoice) }}" method="POST" class="p-6 space-y-6"
                id="invoiceForm">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="payment_status" id="payment_status" required
                                ...
                                <option value="pending"
                                    ...
                                    Pending</option>
                                <option value="paid"
                                    {{ old('payment_status', $invoice->payment_status) == 'paid' ? 'selected' : '' }}>Paid
                                </option>
                                <option value="partial"
                                    {{ old('payment_status', $invoice->payment_status) == 'partial' ? 'selected' : '' }}>
                                    Partial</option>
                        ...
                        @foreach ($invoice->items as $index => $item)
                            <div class="border border-gray-300 rounded-lg p-4 service-item">
                                <div class="flex items-center justify-between mb-3">
                                    <h4 class="font-medium text-gray-900">Service <span
                                            class="service-number">{{ $index + 1 }}</span></h4>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div class="md:col-span-2">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Service Description
                                            *</label>
                                        <input type="text" name="services[{{ $index }}][description]"
                                            value="{{ old('services.' . $index . '.description', $item->service_description) }}"
                                            required placeholder="e.g., Consultation Fee, Medicine, Test, etc."
                                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                    </div>
                                    <div>
                                        ...
                                        <input type="number" step="0.01" min="0"
                                            ...
                                            value="{{ old('services.' . $index . '.fee', $item->fee) }}" required
                ...
                <!-- Consultation Information (Read-only) -->
                <div class="space-y-4">
                    <h3 class="text-lg font-medium text-gray-900 border-b pb-2">Consultation Information</h3>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-4 rounded-lg">
                        <div>
                            <p class="text-sm text-gray-500">Patient</p>
                            <p class="font-medium text-gray-900">{{ $invoice->consultation->appointment->patient->name }}
                            </p>
                            <p class="text-sm text-gray-600">{{ $invoice->consultation->appointment->patient->phone }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-medium text-gray-900">{{ $invoice->consultation->created_at->format('M d, Y') }}
                            </p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Total Services</p>
                            <p class="font-medium text-gray-900">{{ $invoice->items->count() }}</p>
                        </div>
                    </div>
                </div>
    ...
    @push('scripts')
        <script>
            let serviceCount = {{ $invoice->items->count() }};
    ...
    `;
            ...
            document.getElementById('addService').addEventListener('click', function() {
                ...
                serviceCount++;
                serviceElement.querySelector('.service-number').textContent = serviceCount;
            ...
            // Add remove event listeners to existing service items
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Add fee change listeners to existing items
                document.querySelectorAll('.service-fee').forEach(input => {
